...
def display_sidebar(data: pd.DataFrame) -> Tuple[List[str], List[str], List[str], Tuple[datetime.date, datetime.date]]:
    ...
    start_date = pd.Timestamp(st.sidebar.date_input("Start date", min_start_date, min_value=min_start_date, max_value=max_end_date))
    end_date = pd.Timestamp(st.sidebar.date_input("End date", max_end_date, min_value=min_start_date, max_value=max_end_date))
...
def display_charts(data: pd.DataFrame):
    ...
    if CPIH_CPI_separate:
        ...
        fig = px.line(grouped_df, x='Date', y='Value', color='Name',
        ...
        if combine_CPIH_lines:
            ...
            fig = px.area(grouped_data, x='Date', y='Value',
                        title="Customer Price Index Over Time", width=900, height=500)
        else:
            fig = px.area(CPIH_data, x='Date', y='Value', color='Index Name',
                        title="Customer Price Index Over Time", width=900, height=500)
        ...
        if combine_CPI_lines:
            ...
            fig = px.area(grouped_data_CPI, x='Date', y='Value',
                        title="Customer Price Index Over Time", width=900, height=500)
        else:
            fig = px.area(CPI_data, x='Date', y='Value', color='Index Name',
                        title="Customer Price Index Over Time", width=900, height=500)
...
def main():
    ...
    st.title("The Consumer Prices Index including owner occupiers' housing costs by division levels")

    st.markdown("The Consumer Price Index measures the overall change in consumer prices based on a representative basket of goods and services over time.")
